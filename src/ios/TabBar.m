...
- (void)create:(CDVInvokedUrlCommand*)command
{
    ...
    if(options && options != [NSNull null])
    {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7){
            [tabBar setTintColor:[self colorStringToColor:iconTint]];
        }
    }
...
/**
 * Set badge on item.
 * @brief set badge on item.
 * @param arguments the name of the tab bar item to modify and the badge value
 */
- (void)addBadgeOnItem:(CDVInvokedUrlCommand*)command
{
    ...
    NSString *itemName = [command.arguments objectAtIndex:0];
    NSString *badgeValue = [command.arguments objectAtIndex:1];
    UITabBarItem *item = [tabBarItems objectForKey:itemName];
...
/**
 * Clear badge on item
 * @brief Clear badge on item
 * @param arguments the name of the tab bar item to clear
 */
